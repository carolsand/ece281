...
uint8_t InitTemplateService(uint8_t Priority) {
    ...
    // that needs to occur.
    ES_Timer_InitTimer(LIGHT_SERVICE_TIMER, TIMER_1_TICKS);
    ...
    switch (ThisEvent.EventType) {
        ...
        case ES_TIMEOUT:
            ...
            if (currentLightState != lastLightState) { //event detected
                ReturnEvent.EventType = currentLightState;
                ReturnEvent.EventParam = currentLightValue;
